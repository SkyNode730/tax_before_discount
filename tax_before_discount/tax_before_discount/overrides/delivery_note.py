...
def calculate_tax_before_discount(doc, method):
    ...
    Note: Delivery Note taxes affect the total_amount shown on
    the DN but do NOT post accounting entries — those happen on
    the linked Sales Invoice. This override ensures the DN totals
    are consistent with the tax-before-discount policy.
    """
...
def _has_discount(doc):
    """
    Returns True if any discount exists at document or item level.
    Covers:
      - doc.additional_discount_percentage  (document-level %)
      - doc.discount_amount                 (document-level flat)
...
def _recalculate_totals(doc):
    """
    Recomputes document-level total fields after tax rows are adjusted.

    Delivery Note does NOT have outstanding_amount or disable_rounded_total
    — those are invoice concepts. DN only has grand_total and rounded_total
    for display purposes on the print format.
    """
